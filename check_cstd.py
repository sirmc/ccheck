...
for i in subprocess.check_output(["find", ".", "-name", "*.[ch]"]).decode('utf-8').split('\n'):
    ...
    if max((len(s) for s in contents.split('\n'))) > 80:
        die(i + ": line exceeding 80 characters")

    if re.search('[\v\r\t\f]', contents):
        die(i + ": invalid whitespace character in source")

    if re.search('[^\n\x20-\x7e]', contents):
        die(i + ": non-printable character in source")

    if re.search(' $', contents, re.M):
        die(i + ": trailing whitespace")
    ...
    if re.search(';([^ \n]|  +)', canon, re.M):
        die(i + ": semicolon not at end of line and not followed by exactly one space")

    if re.search(r'[^ ] +[];)]', canon):
        die(i + ": semicolon or closing )/] separated from previous token with a space")

    if re.search(r'[[(] +', canon):
        die(i + ": opening (/[ separated from next token with a space")

    if re.search(r'( (\+\+|--|[~!]) )|([^ ([](\+\+|--|[~!])[^] ;)])|(= +\* +)|([^ ([&]&[^] &)])', canon):
        die(i + ": improperly spaced operator")
    ...
    for row, line in enumerate(canon.split('\n')):
        for col, c in enumerate(line):
            ...
            if c == '}':
                if len(dstack) is 0:
                    die(i + ": closing } without opening {")
                elif col != dstack.pop():
                    die(i + ": closing } not aligned with opening {")
